...
def findGoogleSpreadsheet(name, gd_client):
...
def updateKML():   
    ...
    def setColorScheme(kml, gd_client):
        def readColorMap():
          goog = findGoogleSpreadsheet(color_key, gd_client)
          out_dict = {}
          rows = gd_client.GetListFeed(goog[0], goog[1]).entry
          for row in rows:
              custom = row.custom
              line = {}
              for key in custom: line[key] = custom[key].text
              out_dict[line['code']] = line
          return out_dict
...
\t\t</Style>\n".format(code, cmap[code]["label"],cmap[code]["line"],cmap[code]["poly"])
    ...
    def readKey(kml, gd_client):
      goog = findGoogleSpreadsheet(remote_key, gd_client)
      out_dict = {}
      rows = gd_client.GetListFeed(goog[0], goog[1]).entry
      for row in rows:
          line = {}
...
\t\t\t\t</description>\n"
    ...
    gdc = createGoogleClient()
    setColorScheme(local_file, gdc) 
    key = readKey(remote_key, gdc)
